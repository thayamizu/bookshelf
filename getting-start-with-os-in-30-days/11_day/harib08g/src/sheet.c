...
void sheet_updown(Sheet *sheet, int height)
{
    ...
    if (old > height)
    {
        if (height >= 0)
        {
            ...
            sheet_refreshsub(sheet->sheetctl, sheet->vx0, sheet->vy0, sheet->vx0 + sheet->bxsize, sheet->vy0 + sheet->bysize, height + 1);
        }
        else
        {
        ...
        sheet_refreshsub(sheet->sheetctl, sheet->vx0, sheet->vy0, sheet->vx0 + sheet->bxsize, sheet->vy0 + sheet->bysize, 0);
    }
    else if (old < height)
    {
        if (old >= 0)
        ...
        sheet_refreshsub(sheet->sheetctl, sheet->vx0, sheet->vy0, sheet->vx0 + sheet->bxsize, sheet->vy0 + sheet->bysize, height);
    }
...
void sheet_refresh(Sheet *sheet, int bx0, int by0, int bx1, int by1)
{
    if (sheet->height >= 0)
    {
        sheet_refreshsub(sheet->sheetctl, sheet->vx0 + bx0, sheet->vy0 + by0, sheet->vx0 + bx1, sheet->vy0 + by1, sheet->height);
    }
    return;
}
...
//----
//
void sheet_refreshsub(SheetCtl *sheetCtl, int vx0, int vy0, int vx1, int vy1, int h0)
{
    int h, bx, by, vx, vy;
    int bx0, by0, bx1, by1;
...
void sheet_slide(Sheet *sheet, int vx0, int vy0)
{
    ...
    if (sheet->height >= 0)
    {
        sheet_refreshsub(sheet->sheetctl, old_vx0, old_vy0, old_vx0 + sheet->bxsize, old_vy0 + sheet->bysize, 0);
        sheet_refreshsub(sheet->sheetctl, vx0, vy0, vx0 + sheet->bxsize, vy0 + sheet->bysize, sheet->height);
    }
